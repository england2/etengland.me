...
This writeup skips a few implementation details, like the Helm deployment setup, the UI-to-terminal connection logic, the GitHub Actions pipeline, and the per-pod `NodePort` Services. However, the main ideas are still here, and the source code itself is reasonably legible for any who are curious to learn more.
